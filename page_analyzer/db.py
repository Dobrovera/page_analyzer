...
def get_connection(datsbase_url):
    conn = psycopg2.connect(datsbase_url)
    return conn


def add_to_urls(conn, normalize_name):
...
def get_id_from_urls(conn, normalize_name):
    with conn.cursor() as curs:
        curs.execute(
            'SELECT id FROM urls '
            'WHERE name = (%s);',
...
def get_info_from_url_checks(conn, url_id):
    urls_check = []
    with conn.cursor() as curs:
        try:
            curs.execute(
