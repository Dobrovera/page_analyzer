...
def get_id_from_urls(curs, normalize_name):
    curs.execute(
        'SELECT id FROM urls '
        'WHERE name = (%s);',
        (normalize_name, ))
...
def add_to_url_checks(curs, id):
    if check_request(curs, id):
        url_info = check_request(curs, id)
...
def get_info_from_url_checks(curs, url_id):
    urls_check = []
    try:
        curs.execute(
